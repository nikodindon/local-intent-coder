...
class Session:
    ...
    def save(self):
        ...
        if not data["finished_at"] and data["completed"]:
    ...
    def metrics(self) -> dict:
        """Return metrics for this session."""
        existing = [f for f in self.file_list if os.path.exists(os.path.join(self.output_dir, f))]
        missing = [f for f in self.file_list if not os.path.exists(os.path.join(self.output_dir, f))]
